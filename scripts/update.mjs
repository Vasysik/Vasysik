...
function renderActivity(data, mode) {
  const c = theme(mode), W = 920, H = 312;
  ...
  const x0 = 48, y0 = 71, cell = 12, gap = 4;
  ...
  if (days.length) {
    ...
    s += `<line x1="48" y1="128" x2="872" y2="128" stroke="${c.hair}" stroke-dasharray="3 5"/>`;
  }
  s += `<text x="22" y="82" fill="${c.muted}" class="m small">M</text><text x="22" y="114" fill="${c.muted}" class="m small">W</text><text x="22" y="146" fill="${c.muted}" class="m small">F</text>`;
  s += `<line x1="22" y1="174" x2="898" y2="174" stroke="${c.hair}"/>`;
  s += `<text x="22" y="195" fill="${c.muted}" class="m label">CODE MASS / OWN PUBLIC REPOSITORIES</text>`;

  const langs = data.languages?.slice(0, 5) || [];
  const ly = 210, barX = 22, barW = 876, barH = 12;
  let bx = barX;
  ...
  for (const [i, l] of langs.entries()) {
    ...
    s += `<text x="${tx}" y="251" fill="${fill}" class="m small">${esc(l.name)} ${l.percent.toFixed(1)}%</text>`;
  ...
  s += `<text x="22" y="284" fill="${c.muted}" class="m small">${secondary}</text>`;
  return s + `</svg>`;
}
